...
def log_execution_start(command_args: List[str], env_vars: Optional[Dict[str, str]] = None) -> None:
    ...
    debug_log(f"🐍 Python: {context['python_executable']} (v{context['python_version']})", "INFO", "EXEC")
...
def mask_sensitive_value(key: str, value: str) -> str:
    ...
    sensitive_patterns = [
        ...
        "api_key", "client_secret", "private", "cert", "oauth"
    ]
...
def log_validation_summary(total_validators: int, passed: int, failed: int, execution_time: float) -> None:
    ...
    debug_log(f"   Success rate: {(passed/total_validators*100):.1f}%" if total_validators > 0 else "N/A", "INFO", "SUMMARY")
